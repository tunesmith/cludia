...
func TestValidateAndPersistSeparatesDryRunInvalidAndAppliedWrites(t *testing.T) {
	...
	if result, err := ValidateAndPersist(path, doc, validation.ProfileCludia, false); err != nil || !result.OK() {
		t.Fatalf("dry run = %#v, %v", result, err)
	}
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("dry run created file: %v", err)
	}
	if result, err := ValidateAndPersist(path, doc, validation.ProfileCludia, true); err != nil || !result.OK() {
		t.Fatalf("persist = %#v, %v", result, err)
	}
	invalid := doc.Clone()
	invalid.Statements[0].Text = ""
	if result, err := ValidateAndPersist(path, invalid, validation.ProfileCludia, true); err != nil || result.OK() {
