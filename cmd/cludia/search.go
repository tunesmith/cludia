...
func runSearch(args []string, stdout, stderr io.Writer) error {
	...
	output := searchOutput{
		SchemaVersion: outputSchemaVersion, Profile: profile, Query: search,
	...
	for _, match := range query.SearchStatements(doc, search) {
		output.Matches = append(output.Matches, searchMatchOutput{
			Statement: match.Statement, Isolated: isolated[match.Statement.ID],
	...
	for _, match := range output.Matches {
		...
		fmt.Fprintf(stdout, "\t::%s\t[%s]\t%s\n", match.Truth, strings.Join(match.MatchedFields, ","), match.Text)
	}
